namespace Smoothie.Domain.Entities
{
    public class Food
    {
        public string NDB_No { get; set; }
        public string Shrt_Desc { get; set; }
        public double Water { get; set; }
        public double Energ_Kcal { get; set; }
        public double Protein { get; set; }
        public double Lipid_Tot { get; set; }
        public double Ash { get; set; }
        public double Carbohydrt { get; set; }
        public double Fiber_TD { get; set; }
        public double Sugar_Tot { get; set; }
        public double Calcium { get; set; }
        public double Iron { get; set; }
        public double Magnesium { get; set; }
        public double Phosphorus { get; set; }
        public double Potassium { get; set; }
        public double Sodium { get; set; }
        public double Zinc { get; set; }
        public double Copper { get; set; }
        public double Manganese { get; set; }
        public double Selenium { get; set; }
        public double Vit_C { get; set; }
        public double Thiamin { get; set; }
        public double Riboflavin { get; set; }
        public double Niacin { get; set; }
        public double Panto_Acid { get; set; }
        public double Vit_B6 { get; set; }
        public double Folate_Tot { get; set; }
        public double Folic_Acid { get; set; }
        public double Food_Folate { get; set; }
        public double Folate_DFE { get; set; }
        public double Choline_Tot { get; set; }
        public double Vit_B12 { get; set; }
        public double Vit_A_IU { get; set; }
        public double Vit_A_RAE { get; set; }
        public double Retinol { get; set; }
        public double Alpha_Carot { get; set; }
        public double Beta_Carot { get; set; }
        public double Beta_Crypt { get; set; }
        public double Lycopene { get; set; }
        public double Lut_Zea { get; set; }
        public double Vit_E { get; set; }
        public double Vit_D { get; set; }
        public double ViVit_D { get; set; }
        public double Vit_K { get; set; }
        public double FA_Sat { get; set; }
        public double FA_Mono { get; set; }
        public double FA_Poly { get; set; }
        public double Cholestrl { get; set; }
        public double GmWt_1 { get; set; }
        public string GmWt_Desc1 { get; set; }
        public double GmWt_2 { get; set; }
        public string GmWt_Desc2 { get; set; }
        public double Refuse_Pct { get; set; }
        public int GroupId { get; set; }
        public int Status { get; set; }
        public string Name { get; set; }
        public string Image { get; set; }
        public int BaseUnit { get; set; }
    }
}